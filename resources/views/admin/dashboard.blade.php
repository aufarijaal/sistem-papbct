...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
        ...
        <div class="modal fixed w-full h-full top-0 left-0 hidden items-center justify-center" id="modal-hubung">
            ...
            <div class="modal-container bg-slate-700 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
                ...
                <div class="flex justify-between items-center pb-3">
                    <p class="text-lg font-bold text-white">Hubungkan ID mesin dengan User</p>
                </div>
                ...
                <form action="{{ route('bond') }}" method="POST" id="form-hubung">
                    ...
                    <div class="flex flex-col">
                        <label for="userid" class="mr-2 text-white">User ID</label>
                        <input id="userid-for-hubung" required name="userid" type="text" class="h-10 w-72 pl-2 outline-none rounded-lg bg-slate-900 placeholder:text-slate-700 text-white" placeholder="Masukkan user id">
                    </div>
                ...
                <div class="flex justify-end pt-2">
                    <button onclick="updateBond('bind')" class="px-4 py-1 rounded-lg bg-indigo-200 text-indigo-600 mr-2">Hubungkan</button>
            ...
            <div class="modal-container bg-slate-700 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
              ...
              <div class="modal-content py-4 text-left px-6">
                ...
                <form action="{{ route('addmachineid') }}" method="POST" id="form-tambah" class="flex flex-col gap-3">
                    ...
                    <div class="flex flex-col">
                        <label for="userid" class="mr-2 text-white">User ID</label>
                        <input id="userid" name="userid" type="text" class="h-10 pl-2 outline-none rounded-lg bg-slate-900 placeholder:text-slate-700 text-white" placeholder="Masukkan user id">
                    </div>
                </form>
        ...
        <div id="app">
            <div class="container mx-auto mt-10 mb-2">
                <div class="flex justify-center">
                    <a href="{{ route('logout') }}" class="text-blue-600 bg-blue-200 w-max p-1.5 px-3 rounded-lg mx-auto my-4 cursor-pointer">LOGOUT</a>
                </div>
                <div class="my-4">
                    <h1 class="text-white font-bold text-xl text-center">MANAJEMEN DATA MESIN</h1>
                </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-700 border-b-2 border-gray-200">
                            <tr>
                                ...
                                <th class="w-28 p-3 text-sm font-semibold text-white tracking-wide text-center">User ID</th>
                                <th class="w-28 p-3 text-sm font-semibold text-white tracking-wide text-center">Opsi</th>
                                <th class="w-28 p-3 text-sm font-semibold text-white tracking-wide text-center">Hapus</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($datatables as $datatable)
                                <tr class="border-b border-gray-200 bg-slate-900">
                                    ...
                                    <td class="p-2 text-center text-sm text-white">{{ $datatable->userid == null ? 'kosong' : $datatable->userid}}</td>
                                    <td class="p-2 text-center text-sm">
                                        @if ($datatable->userid == null)
                                            <a onclick="openModalHubung('{{ $datatable->machineid }}')" class="bg-green-200 text-green-600 p-1.5 rounded-lg cursor-pointer" style="font-weight: 550; font-variation-settings: 'wght' 550">HUBUNGKAN</a>
                                        @else
                                            <a onclick="updateBond('{{ $datatable->machineid }}', 'unbind', '{{ $datatable->userid }}')" class="bg-red-200 text-red-600 p-1.5 rounded-lg cursor-pointer" style="font-weight: 550; font-variation-settings: 'wght' 550">COPOT</a>
                                        @endif
                                    </td>
                                    <td class="p-2 text-center text-sm">
                                        <form action="{{ route('deletemachineid') }}" method="POST">
                                            ...
                                            <button type="submit" class="bg-red-200 text-red-600 p-1.5 rounded-lg " style="font-weight: 550; font-variation-settings: 'wght' 550">HAPUS</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
    ...
    <script>
        ...
        const updateBond = (machineid, option, userid = null) => {
            if(option == 'unbind') {
                document.getElementById('userid-for-hubung').value = userid
                document.getElementById('machineid-for-hubung').value = machineid
